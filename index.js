...
app.post("/mobileData/signup",express.json(),async function(request,response){
  const {username,email,password} = await request.body;

  const userFromDB = await client.db("MobilePhones").collection("signUpData").findOne({username:username})
...
app.post("/mobileData/login",express.json(),async function(request,response){
  const {username,email,password} = await request.body;
  const userFromDB = await client.db("MobilePhones").collection("signUpData").findOne({email:email})
  if(!userFromDB){
...
app.get("/mobileData/signup",async function (request,response){
  const data = await client.db("MobilePhones").collection("signUpData").find({}).toArray();
})
